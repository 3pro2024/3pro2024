...
test.describe("学習モード基本表示", () => {
  ...
  test("学習モード画面が正しく表示される（ランク指定なし）", async ({
    page,
  }) => {
  ...
  test("手話詳細画面でLocalStorageに学習履歴が保存される", async ({ page }) => {
    ...
    const learnedList = await page.evaluate(() => {
      return localStorage.getItem("learned-shuwa-list"); // ハイフンに修正
    });
    ...
    const learnedCount = await page.evaluate(() => {
      return localStorage.getItem("learned-shuwa-count"); // ハイフンに修正
    });
...
test.describe("学習モード：詳細表示", () => {
  ...
  test("タイトルへ戻るボタンをクリックすると前のページに戻る", async ({
    page,
  }) => {
    // gotoLearnは不要（beforeEachで既に/learn/にいる）
    await page.waitForTimeout(1000);
